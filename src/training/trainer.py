...
class SRATrainer:
    ...
    def _configure_optimizers(self):
        ...
        no_decay = ["bias", "LayerNorm.weight", "ln_f.weight", "ln1.weight", "ln2.weight", "semantic_anchors", "temperature"]
    ...
    def _update_progressive_top_k(self, epoch):
        """Update top-k for all CSR layers based on progressive schedule."""
        ptk = getattr(self.config.model.csr, 'progressive_top_k', None)
        if ptk is None or not getattr(ptk, 'enabled', False):
            return
        schedule = getattr(ptk, 'schedule', None)
        if not schedule:
            return
        # Find the appropriate top_k for this epoch (1-indexed in config)
        new_top_k = None
        for item in schedule:
            if (epoch + 1) >= item['epoch']:
                new_top_k = item['top_k']
        if new_top_k is None:
            return
        unwrapped = self.accelerator.unwrap_model(self.model)
        for module in unwrapped.modules():
            if isinstance(module, ChamberOfSemanticResonance):
                module.set_top_k(new_top_k)

    def _train_epoch(self, epoch):
        # Progressive top-k scheduling
        self._update_progressive_top_k(epoch)
        ...
        for batch in pbar:
            ...
            with self.accelerator.accumulate(self.model):
                ...
                attention_mask = (batch['input_ids'] != 0)  # True for real tokens, False for PAD
                inputs = {'input_ids': batch['input_ids'], 'attention_mask': attention_mask}
                labels = batch['labels']
    ...
    @torch.no_grad()
    def validate(self):
        """Validation loop with dead expert counting."""
        self.model.eval()
        total_lm_loss = 0.0
        num_layers = self.config.model.n_layers if self.config.model.csr.enabled else 0
        expert_counts = None  # Will be initialized on first batch
        total_tokens = 0
        ...
        for batch in pbar:
            ...
            # Accumulate expert routing counts for dead expert tracking
            if aux_data_list and "topk_indices" in aux_data_list[0]:
                if expert_counts is None:
                    num_experts = self.config.model.csr.num_experts
                    expert_counts = [torch.zeros(num_experts, device=self.device) for _ in range(len(aux_data_list))]
                bs = aux_data_list[0]["topk_indices"].size(0)
                total_tokens += bs
                for layer_idx, aux_data in enumerate(aux_data_list):
                    indices = aux_data["topk_indices"].view(-1)
        ...
        # Log dead expert counts per layer
        if expert_counts is not None:
            total_dead = 0
            total_experts = 0
            threshold = total_tokens * 0.0001  # <0.01% of tokens
            for layer_idx, counts in enumerate(expert_counts):
                dead = (counts < threshold).sum().item()
                total_dead += dead
                total_experts += len(counts)
                logging.info(f"  Layer {layer_idx}: Dead Experts: {int(dead)}/{len(counts)}")
            logging.info(f"  Total Dead: {total_dead}/{total_experts} ({100*total_dead/total_experts:.0f}%)")
    ...
    def load_checkpoint(self, checkpoint_path):
        ...
        # Disable batch seeding for resumed training (anchors already loaded from checkpoint)
        for module in unwrapped_model.modules():
            if isinstance(module, ChamberOfSemanticResonance) and hasattr(module, '_needs_batch_seed'):
