...
def apply_rotary_pos_emb(x, cos, sin):
    # Unsqueeze for correct broadcasting over batch and head dimensions
...
class CustomMHA(nn.Module):
    ...
    def forward(self, x, mask=None, rope_cos=None, rope_sin=None):
        ...
        # Prepare mask for SDPA (additive: 0 for attend, -inf for mask)
        attn_mask = None
        if mask is not None:
             attn_mask = torch.zeros_like(mask, dtype=Q.dtype, device=Q.device)
             # True in the boolean mask means MASK (do not attend)
...
class MicroExpert(nn.Module):
    """
    A small Feed-Forward Network representing a single expert in the CSR layer.
    Structurally identical to the FFN in a standard Transformer.
    """
    def __init__(self, d_model, d_ff, dropout=0.1, activation="gelu"):
        ...
        if activation == "gelu":
            # Use 'tanh' approximation for GELU, which often gives better results
...
def create_causal_mask(seq_len, device):
    ...
    In PyTorch MHA, True or 1 (for bool mask) means the position should NOT be attended to.
    """
