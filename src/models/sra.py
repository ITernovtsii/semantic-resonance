...
class SRABlock(nn.Module):
    ...
    def forward(self, x, mask=None, rope_cos=None, rope_sin=None, attention_mask=None):
        x_norm = self.ln1(x)
        ...
        if self.use_csr:
            ff_output, aux_data = self.ff_or_moe(x_norm, attention_mask=attention_mask)
        else:
...
class SRA(nn.Module):
    ...
    def __init__(self, model_config, vocab_size):
        ...
        gc = getattr(model_config, 'gradient_checkpointing', None)
        self.gradient_checkpointing = gc if gc is not None else False
    ...
    def _init_weights_gpt2_style(self, n_layers):
        # Scale weights of layers that feed into the residual stream (for stability)
    ...
    def forward(self, inputs):
        ...
        for block in self.blocks:
            if self.gradient_checkpointing and self.training:
                # Wrapper needed because checkpoint doesn't support dict returns
                def _block_fn(block, x, mask, rope_cos, rope_sin):
                    return block(x, mask=mask, rope_cos=rope_cos, rope_sin=rope_sin, attention_mask=attention_mask)
                x, aux_data = torch_checkpoint(
                    _block_fn, block, x, mask, rope_cos, rope_sin,
                    use_reentrant=False
                )
            else:
                x, aux_data = block(x, mask=mask, rope_cos=rope_cos, rope_sin=rope_sin, attention_mask=attention_mask)
            if aux_data:
        ...
        # Return list of aux_data for loss computation in Trainer
