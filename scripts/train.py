...
def main(config_path, resume_checkpoint=None):
    ...
    try:
        ...
        if config.model.csr.enabled:
            logging.info(f"SRA Architecture: CSR (MoE) is ENABLED. Experts: {config.model.csr.num_experts}, Top-K: {config.model.csr.top_k}")
        else:
...
if __name__ == '__main__':
    # accelerate launch scripts/train.py --config configs/sra_wikitext103.yaml

    parser = argparse.ArgumentParser(description="Train the SRA model.")
    parser.add_argument("--config", type=str, default="configs/sra_wikitext103.yaml",
